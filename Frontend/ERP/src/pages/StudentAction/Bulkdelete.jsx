...
const BulkDelete = () => {
  ...
  const combinedStudents = students.map(student => {
    ...
    return {
      ...
      batch: student.batch?.name || 'Unknown Batch',
      studentName: `${student.firstName} ${student.lastName}`,
      dateOfBirth: formatDate(student.dateOfBirth), // Format date here
    };
  });
...
const BulkDelete = () => {
  const dispatch = useDispatch();
  const { modifiedCourse: courses = [], batchesByCourse: batches = [], error: batchError } = useSelector((state) => state.courses);
  const { students = [], loading: studentLoading, error: studentError } = useSelector(state => state.students);
  const [selectedCourse, setSelectedCourse] = useState("");
  const [selectedBatch, setSelectedBatch] = useState("");
  const [selectedStudents, setSelectedStudents] = useState([]);
